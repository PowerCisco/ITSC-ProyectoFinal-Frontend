...
const peticionDelete = async()=>{
  await axios.delete(url+Paciente.pacienteId).then(response =>{
    setData(Data.filter(datas=>datas.pacienteId!==Paciente.pacienteId))
  })
}
  ...
  const seleccionarPaciente  = (data, caso)=>{
    setPaciente(data);
    (caso==='Editar')?setModalEditar():abrirCerrarModalEliminar();
  }
  useEffect(() => {
            ...
            <TableBody>
              {Data.map(cons => (
                <TableRow key={cons.pacienteId}>
                  ...
                  <TableCell>
                    <Edit className={styles.iconos} onClick={()=>seleccionarPaciente(cons,'Editar')}/>
                    &nbsp; &nbsp;
                    <Delete className={styles.iconos} onClick= {()=>seleccionarPaciente(cons, 'Eliminar')}/>
